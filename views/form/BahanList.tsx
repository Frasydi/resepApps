...
export default function BahanList({data} : {data : Array<{nama: string,jumlah: string,file: DocumentPickerResponse}>}) {
    return(
        <View style={{gap : 20, alignItems:"center"}}>
            {
                data.map((item, ind) => (
                    <View key={item.nama+""+ind} style={{flex : 1, flexDirection:"row"}}>
                        ...
                        <View style={{justifyContent: "center", alignItems: "center",maxWidth:"60%"}}>
                            <Text style={{ paddingHorizontal: 20, fontSize: 15, fontWeight: "bold", color : "black",flexWrap:"wrap"  }}>{item.jumlah} {item.nama}</Text>
                        </View>
                    </View>
                ))
            }
